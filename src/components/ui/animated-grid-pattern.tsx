...
export function AnimatedGridPattern({
  ...
  return (
    ...
    >
      <style>{`
        @keyframes animated-grid-pattern-pulse {
          0%, 100% { opacity: 0; }
      ...
      <svg x={x} y={y} className="overflow-visible">
        {squares.map(({ pos: [squareX, squareY], id, iteration }, index) => (
          <rect
            ...
            fill={
              palette
                ? palette[(id + iteration) % palette.length]
                : "currentColor"
            }
            strokeWidth="0"
            style={{
              opacity: 0,
              animation: `animated-grid-pattern-pulse ${duration + repeatDelay}s ease-in-out ${index * 0.1}s infinite`,
            }}
